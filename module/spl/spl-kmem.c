...
#define MEMLEAK_LOW  24  /* Inclusive */
...
static inline void leak_add(void *ptr, size_t size)
{
	...
	if (size == 0) {
		...
		printf("SPL: alloc size zero from:\n");
		dump_stack(stack);
	}
	...
	if (size >= MEMLEAK_LOW && size < MEMLEAK_HIGH) {
		...
		getpcstack(stack, MEMLEAK_MAX_STACK);
		if (ml) {
			printf("SPL: leak %p exists ml->size %llu, called %lu. Existing leak stack: \n",
				ptr, ml->size, size);
			dump_stack(ml->stack);
			printf("SPL: New (duplicate) alloc stack:\n");
			dump_stack(stack);
			return;
		}
...
zfs_kmem_free(void *buf, size_t size)
{
	...
	if (buf == NULL) {
		uintptr_t stack[MEMLEAK_MAX_STACK];
		getpcstack(stack, MEMLEAK_MAX_STACK);
		printf("SPL: kmem_free(NULL) from: \n");
		dump_stack(stack);
	}
	...
	if (size >= MEMLEAK_LOW && size < MEMLEAK_HIGH) {
		...
		if (ml) {
			if (ml->size != size) {
				uintptr_t stack[MEMLEAK_MAX_STACK];
				getpcstack(stack, MEMLEAK_MAX_STACK);
				printf("SPL: leak free of %p different size %llu != %lu from:\n",
					ml->addr, ml->size, size);
				dump_stack(stack);
				printf("SPL: allocated from:\n");
				dump_stack(ml->stack);
			}
			...
			uintptr_t stack[MEMLEAK_MAX_STACK];
			getpcstack(stack, MEMLEAK_MAX_STACK);
			printf("SPL: leak free of %p:%lu not in memleak_tree at all?\n",
				buf, size);
				dump_stack(stack);
		}
	}
