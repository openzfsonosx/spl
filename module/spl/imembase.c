...
void* internal_malloc(struct IALLOCATOR *allocator, size_t size)
{
	...
	MALLOC(tr, void *, size, M_TEMP, M_WAITOK);
    return tr;
}
...
void* internal_malloc_size(struct IALLOCATOR *allocator, size_t size)
{
    void *tr;
    kern_return_t kr;
...
void internal_free_size(struct IALLOCATOR *allocator, void *ptr, size_t size)
{
    //printf("[spl] malloc free %llu %p\n", size, ptr);
    kmem_free(kernel_map, ptr, size);
}
...
static void* imem_gfp_alloc(imemgfp_t *gfp)
{
	...
	if (imem_gfp_malloc) {
		lptr = (char*)internal_malloc_size(0, imem_page_size);
		if (lptr == NULL) {
			return NULL;
		}
	}	else {
...
static void imem_gfp_free(imemgfp_t *gfp, void *ptr)
{
	...
	if (imem_gfp_malloc) {
		internal_free_size(0, ptr, imem_page_size);
...
void* ikmem_malloc(size_t size)
{
	...
	if (cache == NULL) {
		lptr = (char*)internal_malloc_size(0, IKMEM_LARGE_HEAD + size);
...
void ikmem_free(void *ptr)
{
	...
	if (*(void**)(lptr - sizeof(void*)) == NULL) {
        ilong size;
		size = *(ilong*)(lptr - sizeof(void*) - sizeof(ilong));
		lptr -= IKMEM_LARGE_HEAD;
		p = (iqueue_head*)lptr;
		imutex_lock(&ikmem_lock);
		iqueue_del(p);
		imutex_unlock(&ikmem_lock);
		internal_free_size(0, lptr, size);
	}	else {
