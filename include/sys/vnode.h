...
#define INO_ZFSTOXNU(ID, ROOT) ((ID)==(ROOT)?INO_ROOT:(INO_ISRESERVED(ID)?INO_MAP+(ID):(ID)))
// This macro relies on *unsigned*.
// If asking for 2, return rootID. If in special range, adjust to
// normal, otherwise, return as-is.
#define INO_XNUTOZFS(ID, ROOT) ((ID)==INO_ROOT)?(ROOT):(INO_ISRESERVED((ID)-INO_MAP))?((ID)-INO_MAP):(ID)
